...
*   **双端一致性架构**：PC 桌面端与移动手机端拥有完全对齐的后台架构、设置逻辑与 cyberpunk 紫黑拟物玻璃视觉风格。
*   **生图/对话 Token 隔离**：支持针对生图（Image）与对话/优化（Chat）端点配置独立的 API Token。同时具备“智能降级共用”机制，即若用户仅输入了其中任意一个 Token，另一个 Token 会自动复用，极具人性化。
*   **一键同步云端模型（🔄 按钮）**：模型输入框右侧提供 `🔄` 按钮，一键发送 `/v1/models` 请求，自动拉取并装载当前中转站支持的所有可用模型列表。
...
*   **已删除历史防崩溃机制**：当手动删除了磁盘上的生成原图时，历史卡片不会消失，而是标记为“已删除 (Deleted)”，点击仍能正常查看、复制及重新应用当时的提示词（Prompt），且**绝不发生闪退崩溃**。
*   **自适应拟物玻璃皮肤**：完美适配 PC 电脑版，移动端玻璃卡片带有半透紫黑背景（`rgba(30, 15, 45, 130)`）与霓虹紫发光边框，视觉极具科技感。
*   **配套 Base64 独立转换器 (GUI)**：项目随附提供一个完全独立的极简图形化 Base64 转换器，支持双向转换（文件 ➜ Base64 与 Base64 ➜ 文件），并集成图片拖放、Data URL 前缀一键生成与 Base64 图片实时渲染预览。
...
3.  **批量并发数量限制**
    *   虽然界面支持 1~10 张图的并发选择，但您的 API 代理商可能有严苛的并发限制或网络超时截断。如果批量生成 5 张以上频繁遇到超时，请适当调低生成数量，或升级您的 API 服务级别。
...
### Base64 独立辅助转换器 (GUI)
为了在使用“图生图”或者其他 API 时能更方便地准备 Base64 素材，我们提供了一个配套的小工具：
1.  在 Release 附件中下载并运行 `base64_converter.exe`。
2.  **文件 ➜ Base64**：支持通过按钮选取文件，也支持直接**拖拽文件**到窗口中。它将自动生成编码，提供一键复制或一键保存为 `.txt`，支持自动加上 Data URL 图片头前缀。
3.  **Base64 ➜ 文件**：直接粘贴 Base64 字符串，如果是标准格式图片，**右下角会自动实时显示图片预览**。点击“保存为文件”即可瞬间解码还原回本地。
...
*   `/base64_converter.py`：独立的辅助 Base64 转换器 GUI 源码。

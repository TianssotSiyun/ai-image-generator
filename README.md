...
一款采用Aero美学设计的 AI 智能图像生成器，支持**文生图**及**图生图**工作流。PC 桌面端（Windows）与移动手机端（Android）体验完全对齐，
...
*   **生图/对话 Token 隔离**：支持针对生图（Image）与对话/优化（Chat）端点配置独立的 API Token。同时具备“智能降级共用”机制，即若用户仅输入了其中任意一个 Token，另一个 Token 会自动复用。
